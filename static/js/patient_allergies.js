$(document).ready(function () {
    ...
    function addPatientallergies(data) {
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("Allergies record has been added successfully!", {"status":"success"});
            table.destroy();
    ...
    function deletePatientallergies(id) {
        var settings = {
            "async": true,
            "crossDomain": true,
...
swal({
 ...
 $.ajax(settings).done(function (response) {
   swal("Deleted!", "Allergies record has been removed.", "success");
            table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getPatientallergies()
        });
    ...
    function updatePatientallergies(data, id) {
        var settings = {
            "async": true,
            "crossDomain": true,
            "url": "patient_allergies/" + id,
        ...
        $.ajax(settings).done(function (response) {
            $('.modal.in').modal('hide')
            $.notify("Allergies Record Updated Successfully", {"status":"success"});
            table.destroy();
            $('#datatable4 tbody').empty(); // empty in case the columns change
            getPatientallergies()
        });
    ...
    function getPatientallergies() {
        ...
        $.ajax(settings).done(function (response) {
            ...
            $('#datatable4 tbody').on('click', '.delete-btn', function () {
                ...
                console.log(data)
                deletePatientallergies(data.pat_id)
            ...
            $('.btn-edit').one("click", function(e) {
                ...
                $('#myModal').modal().one('shown.bs.modal', function (e) {
                    ...
                    if(instance.isValid()){
                        ...
                        updatePatientallergies(jsondata, data.pat_id)
                        }
